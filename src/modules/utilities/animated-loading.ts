...
export class AnimatedLoadingManager {
  ...
  static createSpinner(
    ...
    customConfig?: Partial<SpinnerConfig>
  ): string {
    ...
    if (typeof context === 'string' && context in CONTEXT_SPINNERS) {
      config = { ...CONTEXT_SPINNERS[context as SpinnerContext], ...customConfig };
    } else {
      config = {
   ...
   */
  static updateSpinner(
    spinnerId: string,
    text: string,
    newSpinner?: SpinnerTheme
  ): boolean {
    const spinner = this.activeSpinners.get(spinnerId);
    if (spinner) {
      spinner.text = text;
      if (newSpinner) {
        spinner.spinner = newSpinner as any;
      }
      return true;
    }
    return false;
   ...
   */
  static async createSpinnerSequence(
    steps: Array<{
      context: SpinnerContext | string;
      duration?: number;
  ...
  static createThemedSpinner(
    ...
    const themedConfig: Partial<SpinnerConfig> = {
      ...customConfig,
  ...
  private static getThemeColorForContext(
    ...
    const contextColorMap: Record<string, keyof ThemeColors> = {
        loading: 'primary',
        processing: 'warning',
        analyzing: 'accent',
        generating: 'primary',
    ...
    const colorKey = contextColorMap[context as string] || 'primary';
    return theme[colorKey] || '#00D9FF';
  }
}

// Convenience functions for common spinner operations
export const createLoadingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('loading', { text });

export const createProcessingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('processing', { text });

export const createAnalyzingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('analyzing', { text });

export const createGeneratingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('generating', { text });

export const createExportingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('exporting', { text });

export const createSearchingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('searching', { text });

export const createOptimizingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('optimizing', { text });

export const createValidatingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('validating', { text });

export const createConnectingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('connecting', { text });

export const createBuildingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('building', { text });

export const createTestingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('testing', { text });

export const createDeployingSpinner = (text?: string) => 
  AnimatedLoadingManager.createAndStartSpinner('deploying', { text });
