import { BrandKit, ColorPalette, FontConfiguration } from '../data/theme-types';
...
export class BrandKitManager {
   ...
   */
  static createBrandKit(companyName: string, customColors?: { primary: string; secondary?: string; accent?: string }): BrandKit {
    const normalizedName = companyName.toLowerCase().replace(/[^a-z]/g, '');
    const companyColors = COMPANY_BRAND_COLORS[normalizedName as keyof typeof COMPANY_BRAND_COLORS];
   ...
   */
  static generateBrandTheme(brandKit: BrandKit, personality: keyof typeof BRAND_PERSONALITIES = 'professional'): {
    colors: ColorPalette;
   ...
   */
  static async extractColorsFromLogo(logoUrl: string): Promise<{ primary: string; secondary: string; accent: string }> {
   ...
   */
  private static suggestBrandFonts(companyName: string): { primary: string; secondary?: string } {
   ...
   */
  private static generateHarmoniousColor(primaryColor: string, type: 'surface' | 'border'): string {
   ...
   */
  private static getOptimalTextColor(backgroundColor: string): string {
   ...
   */
  private static findBestFontPairing(fontFamily: string): string {
    const pairings = FontManager.getAllPairings();
    const match = pairings.find(p => 
      p.heading.includes(fontFamily) || p.body.includes(fontFamily)
    );
   ...
   */
  private static createDefaultFontConfig(): FontConfiguration {
    return {
      heading: {
        family: 'Inter',
        weight: 600,
