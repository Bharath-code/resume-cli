import { ResumeData, ResumeStats, UserConfig, ThemeColors } from '../data/types.js';
import chalk from 'chalk';
import { getThemeColors } from './config.js';

type ChalkColor = 'cyan' | 'cyanBright' | 'blue' | 'blueBright' | 'green' | 'greenBright' | 'yellow' | 'yellowBright' | 'magenta' | 'magentaBright' | 'white' | 'red' | 'redBright' | 'gray';

/**
 * Calculate resume statistics
 */
export function calculateResumeStats(resumeData: ResumeData): ResumeStats {
 ...
 */
function parseExperienceDates(dates: string): number {
  try {
 ...
 */
export function displayResumeStats(stats: ResumeStats, config?: UserConfig): void {
  const colors = config ? getStatsColors(config) : {
    primary: 'cyanBright' as ChalkColor,
    secondary: 'blueBright' as ChalkColor,
    accent: 'greenBright' as ChalkColor
  };
  
  const primaryColor = (chalk as any)[colors.primary] || chalk.cyan;
  ...
  statsData.forEach(stat => {
    const statColor = (chalk as any)[stat.color] || chalk.green;
 ...
 */
export function getTechBreakdown(resumeData: ResumeData): Record<string, string[]> {
 ...
 */
export function displayTechBreakdown(resumeData: ResumeData, config?: UserConfig): void {
  ...
  const primaryColor = (chalk as any)[colors.primary] || chalk.cyan;
  const secondaryColor = (chalk as any)[colors.secondary] || chalk.blue;
  const accentColor = (chalk as any)[colors.accent] || chalk.green;
 ...
 */
export function getExperienceTimeline(resumeData: ResumeData): Array<{company: string, title: string, duration: string, years: number}> {
 ...
 */
export function displayExperienceTimeline(resumeData: ResumeData, config?: UserConfig): void {
 ...
 */
function getStatsColors(config: UserConfig): ThemeColors {
