...
import { ASCIIThemeManager, ASCIIThemeConfig } from './ascii-theme-manager.js';
import { ColorPaletteGenerator } from './color-generator.js';
import { FontManager } from './font-manager.js';
import { ResumeTheme, ColorSchemeRequest, Industry } from '../../data/theme-types.js';
...
export class ThemeCustomizationInterface {
  ...
  private static customizationOptions: CustomizationOptions | null = null;

  /**
   * Start interactive theme customization
   */
  static async startCustomization(config: UserConfig): Promise<ResumeTheme | null> {
    ...
    try {
      ...
      const finalTheme = await this.previewAndFinalize(
        baseTheme,
        colorCustomization,
        asciiStyle,
        fontCustomization,
   ...
   * Select base theme
   */
  private static async selectBaseTheme(): Promise<ResumeTheme | null> {
    ...
    if (selectedThemeId === 'back') return null;
   ...
   * Customize colors
   */
  private static async customizeColors(baseTheme: ResumeTheme): Promise<any> {
   ...
   * Generate AI colors
   */
  private static async generateAIColors(): Promise<any> {
   ...
   */
  private static async manualColorSelection(baseTheme: ResumeTheme): Promise<any> {
   ...
   */
  private static async selectPresetColors(): Promise<any> {
    const presets = [
   ...
   * Select ASCII style
   */
  private static async selectASCIIStyle(): Promise<string | null> {
   ...
   * Customize fonts
   */
  private static async customizeFonts(baseTheme: ResumeTheme): Promise<any> {
   ...
   * Customize layout
   */
  private static async customizeLayout(baseTheme: ResumeTheme): Promise<any> {
   ...
   * Customize effects
   */
  private static async customizeEffects(): Promise<any> {
   ...
   */
  private static async previewAndFinalize(
    baseTheme: ResumeTheme,
    colorCustomization: any,
    asciiStyle: string | null,
    fontCustomization: any,
    layoutCustomization: any,
    effectsCustomization: any
  ): Promise<ResumeTheme | null> {
    // Create final theme
    let finalTheme: ResumeTheme = { ...baseTheme };
   ...
   * Show theme preview
   */
  private static showThemePreview(theme: ResumeTheme, asciiStyle: string | null): void {
   ...
   */
  static async quickCustomization(): Promise<ResumeTheme | null> {
