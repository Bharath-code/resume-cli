export class LengthOptimizer {
     ...
     */
    optimizeLength(resume, constraints, options = {
        ...
        let optimizedResume = JSON.parse(JSON.stringify(resume));
        const suggestions = [];
     ...
     */
    removeLowPrioritySections(resume, targetReduction, options) {
        const suggestions = [];
        const sectionsModified = [];
        let wordsReduced = 0;
     ...
     */
    trimExperienceDescriptions(resume, targetReduction, options) {
        const suggestions = [];
        let wordsReduced = 0;
        if (!resume.experience)
            return { resume, wordsReduced, suggestions };
        resume.experience.forEach((exp, index) => {
            if (wordsReduced >= targetReduction || !exp.bullets)
                return;
            const originalBullets = [...exp.bullets];
            const originalWords = exp.bullets.reduce((sum, bullet) => sum + this.countWords(bullet), 0);
            ...
            const newWords = exp.bullets.reduce((sum, bullet) => sum + this.countWords(bullet), 0);
     ...
     */
    optimizeProjectDescriptions(resume, targetReduction, options) {
        const suggestions = [];
        let wordsReduced = 0;
        if (!resume.projects)
            return { resume, wordsReduced, suggestions };
     ...
     */
    condenseOtherSections(resume, targetReduction, options) {
        const suggestions = [];
        const sectionsModified = [];
        let wordsReduced = 0;
     ...
     */
    analyzeLength(resume, targetFormat = 'pdf') {
     ...
     */
    exportOptimization(result, format) {
        switch (format) {
            case 'json':
    ...
    generateTextReport(result) {
        const lines = [
            ...
            ...result.suggestions.map(suggestion => `  - ${suggestion.section}: ${suggestion.description} (${suggestion.wordsReduced} words reduced)`)
        ];
        return lines.join('\n');
    }
    /**
...
//# sourceMappingURL=length-optimizer.js.map