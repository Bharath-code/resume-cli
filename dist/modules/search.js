import { addToSearchHistory } from './config.js';
/**
 * Search within resume data
 */
export function searchResume(resumeData, query) {
    const results = [];
    ...
    if (resumeData.profile.toLowerCase().includes(searchTerm)) {
        ...
        const matchingSentence = sentences.find(sentence => sentence.toLowerCase().includes(searchTerm));
        results.push({
            section: 'profile',
            type: 'partial',
    ...
    resumeData.projects.forEach((project, index) => {
        ...
        if (project.tech.toLowerCase().includes(searchTerm)) {
            ...
            const matchingTech = techs.find(tech => tech.toLowerCase().includes(searchTerm));
            results.push({
                section: 'projects',
                type: 'partial',
    ...
    const personalFields = ['name', 'role', 'location'];
 ...
 */
export function getSearchSuggestions(resumeData) {
 ...
 */
export function filterResultsBySection(results, section) {
    return results.filter(result => result.section === section);
}
/**
 * Group search results by section
 */
export function groupResultsBySection(results) {
    const grouped = {};
