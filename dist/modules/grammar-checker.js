export class GrammarChecker {
    ...
    checkGrammar(resume, options = {
        ...
        Object.keys(resume).forEach(sectionKey => {
            const sectionIssues = this.checkResumeSection(resume[sectionKey], sectionKey, options);
    ...
    /**
      ...
      */
    checkResume(resumeData, options = {}) {
    ...
    checkResumeSection(sectionData, sectionKey, options) {
        const issues = [];
        if (!sectionData) {
            return issues;
        }
    ...
    originalCheckGrammar(resume, options = {
        checkSpelling: true,
        checkGrammar: true,
        checkPunctuation: true,
        checkStyle: true,
        ...
        if (resume.leadership) {
            resume.leadership.forEach((item, index) => {
                issues.push(...this.checkSection('Leadership', `item-${index}`, item, options));
            });
        }
        // Check open source
        if (resume.openSource) {
            resume.openSource.forEach((item, index) => {
                issues.push(...this.checkSection('Open Source', `item-${index}`, item, options));
            });
        }
        return this.generateResult(issues);
    }
    checkSection(section, field, text, options) {
        const issues = [];
    ...
    checkSpelling(text, section, field) {
        const issues = [];
    ...
    checkGrammarRules(text, section, field) {
        const issues = [];
    ...
    checkPunctuation(text, section, field) {
        const issues = [];
    ...
    checkStyle(text, section, field, strictMode) {
        const issues = [];
    ...
    generateResult(issues, sectionResults) {
        const issuesByType = {
    ...
    generateRecommendations(issues, issuesByType, issuesBySeverity) {
     ...
     */
    applySuggestions(resume, issues) {
        const correctedResume = JSON.parse(JSON.stringify(resume));
        ...
        Object.entries(issuesBySection).forEach(([key, sectionIssues]) => {
            const autoCorrectableIssues = sectionIssues.filter(issue => issue.type === 'spelling' ||
