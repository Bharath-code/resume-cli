 ...
 */
export async function generateQRCode(resumeData) {
 ...
 * Get website QR data
 */
async function getWebsiteQRData(resumeData, colors) {
 ...
 * Get email QR data
 */
async function getEmailQRData(resumeData, colors) {
 ...
 * Get phone QR data
 */
async function getPhoneQRData(resumeData, colors) {
    const { phone } = await inquirer.prompt([
        {
            ...
            validate: (input) => input.trim().length > 0 || 'Phone number is required'
        }
    ]);
 ...
 */
async function getLinkedInQRData(resumeData, colors) {
 ...
 * Get GitHub QR data
 */
async function getGitHubQRData(resumeData, colors) {
 ...
 * Get PDF QR data
 */
async function getPDFQRData(resumeData, colors) {
 ...
 * Get vCard QR data
 */
async function getVCardQRData(resumeData, colors) {
 ...
 * Get custom QR data
 */
async function getCustomQRData(resumeData, colors) {
    const { customData } = await inquirer.prompt([
        {
            type: 'input',
            name: 'customData',
            message: 'Enter custom data for QR code:',
            validate: (input) => input.trim().length > 0 || 'Data is required'
        }
    ]);
    return customData;
}
 ...
 */
async function generateAndSaveQR(data, filename, resumeData, colors) {
    ...
    try {
        ...
        console.log(chalk[colors.muted](`📁 Saved to: ${outputPath}`));
        console.log(chalk[colors.muted](`📊 Size: ${size}x${size}px`));
        console.log(chalk[colors.muted](`🔧 Error correction: ${errorLevel}\n`));
    }
    catch (error) {
 ...
 */
export async function exportContactCard(resumeData) {
    const colors = getThemeColors({ theme: 'colorful', favorites: [] });
    console.log(chalk[colors.primary].bold('\n📇 Export Contact Card\n'));
 ...
 */
async function getContactData(personalInfo, colors) {
 ...
 */
function generateHTMLCard(data, config) {
...
//# sourceMappingURL=interactive-utilities.js.map