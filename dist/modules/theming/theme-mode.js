...
export class ThemeModeManager {
     ...
     */
    static initialize(defaultMode = 'auto') {
        if (!isBrowser)
            return;
     ...
     */
    static isValidThemeMode(mode) {
        return mode === 'light' || mode === 'dark' || mode === 'auto';
    }
    /**
     * Convert light colors to dark mode
     */
    static convertToDarkMode(lightColors) {
        return {
     ...
     */
    static convertToLightMode(darkColors) {
        return {
     ...
     */
    static applyThemeToHTML(html, theme, mode) {
     ...
     */
    static generateThemeCSS(colors, fonts, mode) {
        return `
      :root {
        /* Color Variables */
     ...
     */
    static adjustColorForLightMode(color) {
        // Simple color adjustment - in a real implementation, you'd use a color library
        if (color.startsWith('#')) {
            // Convert hex to RGB, adjust, and convert back
            const r = parseInt(color.slice(1, 3), 16);
     ...
     */
    static enableAutoSwitch(lightTime = '06:00', darkTime = '18:00') {
        if (!isBrowser)
            return;
        const checkTime = () => {
            const now = new Date();
     ...
     */
    static extractModeValue(themeMode) {
     ...
     */
    static createThemeMode(mode, autoSwitchTime) {
        return {
            mode,
            autoSwitchTime
        };
...
ThemeModeManager.currentMode = 'auto';
ThemeModeManager.mediaQuery = null;
