...
export class FontManager {
     ...
     */
    static getAllPairings() {
        return FONT_PAIRINGS;
    }
    /**
     * Get font pairings by category
     */
    static getPairingsByCategory(category) {
        return FONT_PAIRINGS.filter(pairing => pairing.category === category);
    }
    /**
     * Find font pairing by name
     */
    static getPairingByName(name) {
        return FONT_PAIRINGS.find(pairing => pairing.name.toLowerCase() === name.toLowerCase());
    }
    /**
     * Suggest font pairings based on industry and style preferences
     */
    static suggestPairings(industry, style) {
        ...
        const preferredCategories = industryMapping[industry] || ['modern'];
        // Get pairings that match preferred categories
        let suggestions = FONT_PAIRINGS.filter(pairing => preferredCategories.includes(pairing.category));
        // If style is specified, prioritize matching styles
        if (style === 'classic') {
            suggestions = suggestions.filter(p => p.category === 'classic').concat(suggestions.filter(p => p.category !== 'classic'));
        }
        else if (style === 'creative') {
            suggestions = suggestions.filter(p => p.category === 'creative').concat(suggestions.filter(p => p.category !== 'creative'));
        }
        else if (style === 'modern') {
            suggestions = suggestions.filter(p => p.category === 'modern').concat(suggestions.filter(p => p.category !== 'modern'));
        }
     ...
     */
    static createConfiguration(pairing, scale = 'standard') {
     ...
     */
    static generateCSS(config, includeGoogleFonts = true) {
        let css = '';
        // Google Fonts import
     ...
     */
    static async validateFont(fontFamily) {
     ...
     */
    static getOptimalWeight(fontFamily, usage) {
        const weightMap = {
            heading: {
        ...
        return weightMap[usage][fontFamily] || weightMap[usage].default;
    }
    /**
     * Get optimal line height for readability
     */
    static getOptimalLineHeight(fontFamily) {
        const lineHeightMap = {
            'Inter': '1.5',
            'Open Sans': '1.6',
            'Source Sans Pro': '1.6',
     ...
     */
    static generateGoogleFontsUrl(fonts) {
        const googleFonts = fonts.filter(font => !SYSTEM_FONTS.serif.includes(font) &&
     ...
     */
    static getFontWeights(fontFamily) {
        const weightMap = {
     ...
     */
    static getFallbacks(fontFamily) {
