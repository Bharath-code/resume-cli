...
export class ColorPaletteGenerator {
     ...
     */
    static generateColorScheme(request) {
     ...
     */
    static checkAccessibility(foreground, background) {
    ...
    static selectPrimaryColor(industryProfile, request, index) {
        let candidates = [...industryProfile.primary];
        // Filter out avoided colors
        if (request.preferences?.avoidColors) {
            candidates = candidates.filter(color => !request.preferences.avoidColors.some(avoid => this.colorDistance(color, avoid) < 50));
        }
        // Prioritize favorite colors
        if (request.preferences?.favoriteColors) {
            const favoriteMatches = candidates.filter(color => request.preferences.favoriteColors.some(fav => this.colorDistance(color, fav) < 30));
    ...
    static buildPalette(primaryColor, personalityBase, request) {
    ...
    static calculateAccessibility(palette) {
    ...
    static generateDescription(industry, personality, index) {
        const descriptions = {
    ...
    static generateReasoning(industryProfile, personality, primaryColor) {
    ...
    static calculateConfidence(request, palette) {
        ...
        if (request.preferences?.favoriteColors) {
            const hasMatch = request.preferences.favoriteColors.some(fav => this.colorDistance(palette.primary, fav) < 50);
            if (hasMatch)
                confidence += 0.1;
        }
