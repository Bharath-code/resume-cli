...
import { GrammarChecker } from './grammar-checker.js';
import { SocialIntegration } from './social-integration.js';
import { GitHubAnalyticsEngine } from './github-analytics.js';
/**
 ...
 */
async function exportContactCard(resumeData) {
    const { format } = await inquirer.prompt([
        {
            type: 'list',
            name: 'format',
 ...
 */
async function analyzeATSScore(resumeData) {
    ...
    const { jobDescription } = await inquirer.prompt([
        {
            ...
            validate: (input) => input.trim().length > 0 || 'Please provide a job description'
        }
    ]);
    try {
...
async function optimizeLength(resumeData) {
...
async function optimizeKeywords(resumeData) {
 ...
 */
async function analyzeGitHubProfile(resumeData) {
    const config = loadConfig();
    const colors = getThemeColors(config);
    ...
    try {
        ...
        console.error(chalk.red('❌ Error fetching GitHub data:'), error);
        console.log(chalk[colors.warning]('\n💡 Tips:'));
