...
export class ThemeCLI {
    ...
    async createCustomTheme(options) {
        try {
            let request;
    ...
    async createBrandTheme(options) {
        try {
            let brandKit;
            let personality;
            if (!options.colors || !options.personality) {
                const answers = await inquirer.prompt([
                    {
                        ...
                        validate: (input) => {
                            ...
                            return colors.every((c) => /^#[0-9A-F]{6}$/i.test(c)) || 'Please enter valid hex colors';
                        }
                    },
                    {
                        type: 'list',
                ...
                const colors = (options.colors || answers.colors).split(',').map((c) => c.trim());
                brandKit = {
                    colors: {
                        primary: colors[0],
                        secondary: colors[1],
                ...
                const colors = options.colors.split(',').map((c) => c.trim());
                brandKit = {
                    colors: {
                        primary: colors[0],
                        secondary: colors[1],
            ...
            const theme = ThemeEngine.createFromBrandKit(brandKit, personality);
            await this.saveTheme(theme, options.output);
    ...
    async previewTheme(options) {
      ...
      `;
            const html = ThemeEngine.applyThemeToHTML(sampleHTML, theme, options.mode);
    ...
    async saveTheme(theme, outputDir) {
